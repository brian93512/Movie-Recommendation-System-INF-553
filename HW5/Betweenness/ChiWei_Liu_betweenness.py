...
for v in result:
	fileToOpen.write("(" + "%s,%s,%s" % (v[0][0],v[0][1],v[1]) + ")"+ "\n") 
fileToOpen.close()
#########
...
# #edg = allEdges.flatMap(lambda x: getWeight(x[0],x[1])).flatMap(lambda x: x).groupByKey().map(lambda x: getAvg(x[0],x[1])).flatMap(lambda x: x)  #.collect()#.flatMap(lambda x: x).collect()
# edg = allEdges.flatMap(lambda x: getWeight2(x[0],x[1])).flatMap(lambda x: x).groupByKey().reduceByKey() #.flatMap(lambda x: x)  #.collect()#.flatMap(lambda x: x).collect()
########
# edg = allEdges.flatMap(lambda x: getWeight2(x[0],x[1])).flatMap(lambda x: x).groupByKey().flatMap(lambda x: getAvg(x[0],x[1]))
# m = edg.count()
# cutOrder = edg.map(lambda x: (x[1],x[0])).sortByKey(False).map(lambda x: x[1]).collect()
